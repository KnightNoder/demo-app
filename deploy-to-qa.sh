...
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no -P 4993 "${QA_SERVER_USER}@${QA_SERVER_HOST}" "sudo chmod 644 $QA_DEPLOY_PATH/$JS_FILENAME"
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no -P 4993 "${QA_SERVER_USER}@${QA_SERVER_HOST}" "sudo chmod 644 $QA_DEPLOY_PATH/$CSS_FILENAME"  
echo "Changed permissions of JS and CSS files to 644"

# Change the ownership of the JS and CSS files to www-data
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no -P 4993 "${QA_SERVER_USER}@${QA_SERVER_HOST}" "sudo chown www-data:www-data $QA_DEPLOY_PATH/$JS_FILENAME"
sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no -P 4993 "${QA_SERVER_USER}@${QA_SERVER_HOST}" "sudo chown www-data:www-data $QA_DEPLOY_PATH/$CSS_FILENAME"
