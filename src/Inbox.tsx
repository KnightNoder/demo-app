...
const Inbox = () => {
  ...
  const fetchAllData = useCallback(
    async (isInitialLoad = false) => {
      console.log("fetchAllData called, isInitialLoad:", isInitialLoad);
      try {
        if (isInitialLoad) {
          setLoading(true);
        }
        ...
        const results = await Promise.allSettled([
          ...
          fetchApplicantCount().then(setApplicantCount),
        ]);
  ...
  // Create stable reference for polling function
  const pollingFetchFunction = useCallback(() => fetchAllData(false), [fetchAllData]);

  // Use polling hook
  const polling = usePolling({
    pollingInterval,
    isPolling,
    fetchDataFunction: pollingFetchFunction,
  });
                    ...
                    <div className="p-2">
                      <div className="flex gap-1">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1 px-2 py-1 text-xs text-gray-700 hover:bg-gray-50 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Refresh now"
