...
// import {
  // PlayIcon,
// } from "../../../../src/Inbox/components/assets/Icons/PlayIcon";
...
import { TaskManagementContainer } from "./TaskManagementContainer2";
import axiosClient from "../../../api/axiosClient";
...
const Inbox = () => {
  ...
  // Fetch birthday data from API (now used for initial data only)
  const fetchBirthdayData = async () => {
    try {
      ...
      const response = await axiosClient.get<BirthdayApiResponse>(
        ...
        {
          params: {
            per_page: 20, // Reduced since we now use server-side pagination
            page: 1,
          },
        }
      );
  ...
  const handleCardClick = (cardType: string) => {
    ...
    if (cardType === "Birthdays") {
      ...
      console.log(`Handling ${cardType} card`);
    }
  };
  ...
  return (
    <div className="w-full bg-[#f4f5fb] text-[#020817]">
      ...
      <div className="space-y-2 md:space-y-4 px-4">
        ...
        {/* Add New Label Button - Commented out as in original */}
      ...
      {/* Task Management Container with TanStack Table */}
      <div className="w-full bg-[#f4f5fb]">
        <TaskManagementContainer
          ...
          apiEndpoint="/inbox/birthdays"
          useServerSide={true}
        />
      </div>

      {/* TaskSlidePanel - Commented out as in original */}
      {/* <TaskSlidePanel
